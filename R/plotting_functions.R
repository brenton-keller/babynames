...
plot_name <- function(name_input, years = 1970:2024, sex_input = c("M","F")) {
  d <- copy(babynames$national)[, n_prop := round(n/sum(n)*100, 3), by=.(year, sex)][
    name %in% name_input & year %in% years & sex %in% sex_input]
  if (!nrow(d)) stop("No data found for specified criteria")

  pal <- list(M = colorRampPalette(c("#c6dbef", "#08519c")),
              F = colorRampPalette(c("#fbb4b9", "#7a0177")))
  map <- d[, .(name = sort(unique(name))), by = sex][, col := pal[[sex[1]]](.N), by = sex]
  d <- map[d, on = .(sex, name)]

  plot(NA, xlim = range(d$year), ylim = range(d$n_prop),
       xlab = "Year", ylab = "Percentage of births (%)",
       main = paste("Popularity of", paste(name_input, collapse = ", ")))
  d[order(sex, name, year), { lines(year, n_prop, col = col[1], lwd = 2)
                              points(year, n_prop, col = col[1], pch = 16, cex = 0.5) },
    by = .(sex, name)]

  if (uniqueN(d$sex) > 1L || uniqueN(d$name) > 1L) {
    lg <- unique(d[, .(sex, name, col)])[order(sex, name)]
    legend("topright", legend = paste0(lg$name, " (", lg$sex, ")"),
           col = lg$col, lwd = 2, cex = 0.8, bty = "n")
  }
}
...
plot_name <- function(name_input, years = 1970:2024, sex_input = c("M","F")) {
  ...
  # Plot lines
  d[order(year), {
    lines(year, n_prop, col = col[1], lwd = 2.6)
    points(year, n_prop, col = col[1], pch = 19, cex = 0.4)
  }, by = .(sex, name)]
  ...
  group_peaks_for_labels <- d[, .SD[which.max(n_prop)], by = .(sex, name)][
    , .SD[which.max(n_prop)], by = group_rank
  ][order(-n_prop)]
...
detect_breakpoints <- function(dt, name, sex = "M", years = 1970:2024, h = 0.15) {
  ...
  data <- get_name_stats(dt, years)[
    name == name_val & sex == sex_val][order(year)]
  
  if (nrow(data) < 10) stop("Insufficient data points")
  
  ts_data <- ts(data$prop * 100, start = min(data$year))
  bp_test <- strucchange::breakpoints(ts_data ~ 1, h = h)
  
  plot(data$year, data$prop * 100, type = "l", lwd = 2,
       main = paste("Breakpoints:", name, "(", sex, ")"),
       xlab = "Year", ylab = "Percentage (%)")
  
  cat("\n===== BREAKPOINT ANALYSIS =====\n")
  cat("Name:", name, "(", sex, ")\n")
  cat("Period:", min(data$year), "-", max(data$year), "\n")
  cat("Range:", sprintf("%.3f%% - %.3f%%", min(data$prop * 100), max(data$prop * 100)), "\n\n")
  
  if (length(bp_test$breakpoints) > 0) {
    bp_years <- data$year[bp_test$breakpoints]
    abline(v = bp_years, col = "red", lwd = 2, lty = 2)
    
    cat("BREAKPOINTS DETECTED:", length(bp_years), "\n")
    cat("Years:", paste(bp_years, collapse = ", "), "\n\n")
    
    # Analyze segments
    segments <- c(1, bp_test$breakpoints, nrow(data))
    for (i in 1:(length(segments) - 1)) {
      ...
      # Linear trend
      lm_fit <- lm(prop ~ year, data = seg_data)
      slope <- coef(lm_fit)[2] * 100 * 1000  # per 1000 years for readability
      
      cat(sprintf("Segment %d (%d-%d):\n", i, start_year, end_year))
      cat(sprintf("  Trend: %+.3f%% per decade\n", slope / 100))
      cat(sprintf("  Start: %.3f%% → End: %.3f%%\n", 
                  seg_data$prop[1] * 100, seg_data$prop[nrow(seg_data)] * 100))
      
      direction <- if (slope > 0.1) "Rising" else if (slope < -0.1) "Declining" else "Stable"
    ...
    lm_fit <- lm(prop ~ year, data = data)
    slope <- coef(lm_fit)[2] * 100 * 1000
    cat(sprintf("Overall trend: %+.3f%% per decade\n", slope / 100))
